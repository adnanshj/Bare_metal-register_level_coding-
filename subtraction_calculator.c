//PERFORM SUBTRACTION AND IF EXCEEDS 9999 THEN DISPLAY EEEE,AND ALSO DISPLAY NEGATIVE NUMBERS ALSO.
//PRESS B FOR SUBTRACTION AND # FOR GETTING ANSWER
//GLOBAL VARIABLE DECLARATION
volatile char *row,*col,*disp,*sel;
volatile short mem[10]={0};
volatile long k;
volatile short arr[12]={0x40,0x79,0x24,0x30,0x19,0x12,0x02,0x78,0x00,0x10,0x06,0x3f},arr2[4]={0};
volatile int x=0,y=0,i,j=0,z,power=1,cut=0;
...
if(*col!=0){
      ...
      if(i==1 && *col==0x08) mem[j]=0x0a;// b press
...
if(z>9999){
  ...
  if(z<0) z=-(z); //making z value positive 
  for(i=3;i>=0;i--){
    arr2[i]=z%10;
    z=z/10;
  }
  z=y; //reviving z value
  if(z<0){
    arr2[0]=11;//if z value is negative then display negative sign
  }
}
}
